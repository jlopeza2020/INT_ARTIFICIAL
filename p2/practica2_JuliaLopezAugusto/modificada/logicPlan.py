...
def sentence4():
    "*** MODIFICATION 1 ***"
    # Inicializo las 4 expresiones lógicas 
    Flecha_0 = logic.PropSymbolExpr("AgenteFlecha", time=0)
    Flecha_1 = logic.PropSymbolExpr("AgenteFlecha", time=1)
    Dispara_0 = logic.PropSymbolExpr("AgenteDispara", time=0)
    Wumpus_muerto_1 = logic.PropSymbolExpr("WumpusMuerto", time=1)

    # Defino las sentencias
    # El Agente tiene flechas en el instante 1 si y solo si tiene flechas en el instante 0 y no
    # dispara flecha en el instante 0
    def_1 = Flecha_1 % ( Flecha_0 & ~Dispara_0)
    # Que el Agente dispare en el instante 0 y no tenga flecha en el instante 1 implica que el
    # Wumpus está muerto en el instante 1.
    def_2  = (Dispara_0 & ~ Flecha_1) >> Wumpus_muerto_1

    # Que el Wumpus no esté muerto en el instante 1, el Agente no dispare en el instante 0 y el
    # gente tenga flecha en el instante 0 implica que el Agente tiene flecha en el instante 1.
    def_3 = (~Wumpus_muerto_1 & ~Dispara_0 & Flecha_0) >> Flecha_1


    # El Agente no dispara la flecha en el instante 0 y el Wumpus no está muerto en el tiempo 1
    # y el Agente tiene flecha en el instante 1.
    def_4 = ~Flecha_0 & ~Wumpus_muerto_1 & Flecha_1

    # las encadeno en orden 
    return logic.conjoin(def_1, def_2, def_3, def_4) 

def sentence5():

    "*** MODIFICATION 2 ***"
    # Inicializo las 4 expresiones lógicas
    Agent_0 = logic.PropSymbolExpr("Agente", 1, 1, time=0)
    Agent_1 = logic.PropSymbolExpr("Agente", 2, 1, time=1)
    Norte_0 = logic.PropSymbolExpr("Norte", time=0)
    Este_0 = logic.PropSymbolExpr("Este", time=0)

    # El Agente está en la posición (2,1) en el tiempo 1 si y solo si estaba en la posición (1,1) en
    # el tiempo 0 y se movió hacia el Este en el tiempo 0 y no se movió hacia el Norte en el
    # tiempo 0.
    def_1 = Agent_1 % ( Agent_0 & Este_0 & ~Norte_0)

    # El Agente está en la posición (2,1) en el tiempo 1
    def_2 = Agent_1

    # El Agente en el tiempo 0 solo se ha podido mover al Este o al Norte
    def_3 = Agent_0 >> (Este_0 | Norte_0)

    # las encadeno en orden 
    return logic.conjoin(def_1, def_2, def_3)
